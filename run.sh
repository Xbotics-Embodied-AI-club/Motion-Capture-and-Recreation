...
# Online global SLAM (DPVO): ground WHAM motion in the world frame for a moving camera.
GLOBAL_SLAM=${GLOBAL_SLAM:-0}
...
echo "[E2E] Global SLAM: enabled=${GLOBAL_SLAM}"
...
if [[ "${GLOBAL_SLAM}" == "1" ]]; then
	INTEGRATED_CMD+=(--global_slam)
fi
