...
def run(cfg,
    ...
    with torch.no_grad():
        if not (osp.exists(osp.join(output_pth, 'tracking_results.pth')) and 
            ...
            # Use lightweight detector (ViTPose-B + YOLOv8n) so the whole pipeline
            # fits in an 8GB GPU. The defaults (ViTPose-H + YOLOv8x) OOM on small cards.
            root_dir = os.path.dirname(os.path.abspath(__file__))
            pose_model_cfg = osp.join(root_dir, 'third-party/ViTPose/configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco/ViTPose_base_coco_256x192.py')
            pose_model_ckpt = osp.join(root_dir, 'checkpoints/vitpose_base_coco_aic_mpii.pth')
            bbox_model_ckpt = osp.join(root_dir, 'checkpoints/yolov8n.pt')
            detector = DetectionModel(cfg.DEVICE.lower(), pose_model_cfg=pose_model_cfg,
                                      pose_model_ckpt=pose_model_ckpt, bbox_model_ckpt=bbox_model_ckpt)
