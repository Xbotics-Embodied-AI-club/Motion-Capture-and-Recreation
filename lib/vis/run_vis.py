...
def run_vis_on_demo(cfg, video, results, output_pth, smpl, vis_global=True):
    ...
    while (cap.isOpened()):
        ...
        if vis_global:
            ...
            # Keep a single view so the output video matches the input video's
            # resolution/aspect ratio. (Previously the global view was stacked
            # side-by-side, doubling the width and changing the aspect ratio.)
