...
class DetectionModel(object):
    def __init__(self, device, pose_model_cfg=None, pose_model_ckpt=None, bbox_model_ckpt=None):

        # ViTPose
        if pose_model_ckpt is None:
            pose_model_ckpt = osp.join(ROOT_DIR, 'checkpoints', 'vitpose-h-multi-coco.pth')

        if pose_model_cfg is None:
            ckpt_name = osp.basename(str(pose_model_ckpt)).lower()
            cfg_dir = osp.join(
                VIT_DIR,
                'configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco',
            )
            # Match multi-dataset checkpoints with vitPose+ configs to reduce checkpoint mismatch warnings.
            cfg_candidates = []
            if ('multi' in ckpt_name) or ('aic' in ckpt_name) or ('mpii' in ckpt_name):
                if ('base' in ckpt_name) or ('-b' in ckpt_name):
                    cfg_candidates.append('vitPose+_base_coco+aic+mpii+ap10k+apt36k+wholebody_256x192_udp.py')
                elif ('large' in ckpt_name) or ('-l' in ckpt_name):
                    cfg_candidates.append('vitPose+_large_coco+aic+mpii+ap10k+apt36k+wholebody_256x192_udp.py')
                elif ('small' in ckpt_name) or ('-s' in ckpt_name):
                    cfg_candidates.append('vitPose+_small_coco+aic+mpii+ap10k+apt36k+wholebody_256x192_udp.py')
                else:
                    cfg_candidates.append('vitPose+_huge_coco+aic+mpii+ap10k+apt36k+wholebody_256x192_udp.py')

            # Fallback to the original default if no matched config is found.
            cfg_candidates.append('ViTPose_huge_coco_256x192.py')

            pose_model_cfg = None
            for cfg_name in cfg_candidates:
                cfg_path = osp.join(cfg_dir, cfg_name)
                if osp.exists(cfg_path):
                    pose_model_cfg = cfg_path
                    break
            if pose_model_cfg is None:
                pose_model_cfg = osp.join(cfg_dir, 'ViTPose_huge_coco_256x192.py')

        self.pose_model = init_pose_model(pose_model_cfg, pose_model_ckpt, device=device.lower())

        # YOLO
        if bbox_model_ckpt is None:
