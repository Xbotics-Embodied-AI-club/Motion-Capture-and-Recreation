...
def _align_betas_to_model(raw_betas, body_model):
    betas = _to_numpy(raw_betas).astype(np.float32).reshape(-1)
...
def _apply_yup_to_zup(root_orient, trans):
...
def _should_apply_yup_to_zup(smplx_data, smplx_file, coord_fix):
    ...
    file_name = os.path.basename(str(smplx_file)).lower()
    if file_name.startswith("gmr_smplx"):
        return True

    if hasattr(smplx_data, "files"):
        meta = []
        for k in ("coord_system", "source", "producer"):
            if k in smplx_data.files:
                meta.append(_scalar_text(smplx_data[k]).lower())
        meta_text = " ".join(meta)
        if ("y-up" in meta_text) or ("y_up" in meta_text) or ("wham" in meta_text):
            return True
...
def load_smplx_file(smplx_file, smplx_body_model_path, coord_fix="auto"):
    ...
    root_orient = _to_numpy(smplx_data["root_orient"]).astype(np.float32).reshape(num_frames, 3)
    trans = _to_numpy(smplx_data["trans"]).astype(np.float32).reshape(num_frames, 3)

    if _should_apply_yup_to_zup(smplx_data, smplx_file, coord_fix):
        root_orient, trans = _apply_yup_to_zup(root_orient, trans)
        print("[SMPL] Applied coordinate fix: y-up -> z-up")

    betas = _align_betas_to_model(smplx_data["betas"], body_model)
...
def load_gvhmr_pred_file(gvhmr_pred_file, smplx_body_model_path):
    ...
    betas = _align_betas_to_model(smpl_params_global['betas'][0], body_model)
    ...
    smplx_data = {
        'pose_body': _to_numpy(smpl_params_global['body_pose']),
        'betas': betas,
        'root_orient': _to_numpy(smpl_params_global['global_orient']),
        'trans': _to_numpy(smpl_params_global['transl']),
...
def get_smplx_data_offline_fast(smplx_data, body_model, smplx_output, tgt_fps=30):
    ...
    if tgt_fps < src_fps:
        ...
        # Keep source timing when target FPS is not lower than source FPS.
