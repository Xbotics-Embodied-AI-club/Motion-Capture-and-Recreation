...
class RobotMotionViewer:
    def __init__(
        ...
        # Video output resolution. Default to the WHAM input video's portrait
        # resolution (720x1280) so recorded robot videos match the source video's
        # aspect ratio. Override via GMR_VIDEO_WIDTH / GMR_VIDEO_HEIGHT.
        video_width = int(video_width) if video_width is not None else int(os.environ.get("GMR_VIDEO_WIDTH", "720"))
        video_height = int(video_height) if video_height is not None else int(os.environ.get("GMR_VIDEO_HEIGHT", "1280"))
        ...
        if self.record_video:
            ...
            try:
                # MuJoCo's offscreen framebuffer defaults to 1080px tall; our
                # portrait video is 1280px tall, so enlarge the framebuffer to
                # match (otherwise mj.Renderer raises "height > framebuffer").
