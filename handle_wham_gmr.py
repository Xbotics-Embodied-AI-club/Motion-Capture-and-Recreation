...
try:
    from scripts.online_slam import OnlineSLAM
...
def run_stream_mt(
    ...
    if is_cuda_device:
        # cudnn.benchmark=True makes cuDNN allocate ~2 GB of extra workspace to
        # benchmark every new conv shape, which OOMs the 8 GB card when the
        # global-SLAM (DPVO) thread runs alongside WHAM.  Heuristic algorithm
        # selection is plenty fast here and keeps peak VRAM low.
    ...
    # Release the fp32 weight blocks freed by the .half() conversion above back to
    # the GPU.  Otherwise PyTorch keeps ~1.5 GB of now-unused fp32 weights cached,
    # which OOMs the 8 GB card once the global-SLAM (DPVO) thread is running too.
    if is_cuda_device:
    ...
    # ---- Online global SLAM (DPVO) ----
    # Feeds camera angular velocity into WHAM's trajectory decoder so the
    # estimated human motion is grounded in the world frame (moving camera).
    slam_obj = None
    slam_Q = None
    slam_thread = None
    slam_dropped = 0
    if global_slam:
        if not _SLAM_AVAILABLE:
            logger.warning("[SLAM] --global_slam requested but online_slam module unavailable; "
                           "falling back to local motion.")
        else:
            slam_scale = max(0.1, float(os.environ.get("SLAM_SCALE", "0.5")))
            slam_buffer = max(256, int(os.environ.get("SLAM_BUFFER_SIZE", "1024")))
            slam_Q = Queue(maxsize=max(8, int(os.environ.get("SLAM_QUEUE_SIZE", "32"))))
            slam_obj = OnlineSLAM(width, height, scale=slam_scale, buffer_size=slam_buffer)
            logger.info(f"[SLAM] online global SLAM enabled: scale={slam_scale:.2f} buffer={slam_buffer}")
    ...
    def reader_thread():
        ...
        def tee_slam(fid, fr):
            if slam_Q is None:
                return
            nonlocal slam_dropped
            try:
                slam_Q.put((fid, fr), timeout=0.1)
            except Full:
                slam_dropped += 1
            ...
            if track_id is not None:
                if track_id not in temporal_history:
                    temporal_history[track_id] = {
                        ...
                        'frame_ids': deque(maxlen=stream_seq_len),
                        'init_data': init_data,
                        'cached_inits': None,
                ...
                hist['frame_ids'].append(frame_id)
                ...
                if slam_obj is not None:
                    cam_angvel = slam_obj.cam_angvel_for(list(hist['frame_ids']), fps, device)
    ...
    def slam_thread_worker():
        while not stop_event.is_set():
            try:
                data = slam_Q.get(timeout=0.1)
            except Empty:
                continue
            if data is None:
                break
            fid, frame = data
            slam_obj.process(frame, fid)
        if slam_obj is not None:
            slam_obj.close()
    ...
    if slam_Q is not None:
        slam_thread = threading.Thread(
            target=lambda: guarded_thread("slam", slam_thread_worker),
            daemon=True,
            name="slam",
        )
        slam_thread.start()
    ...
    def init_gmr_state(state, betas, create_viewer=None):
        ...
        if state["retarget"] is not None:
            ...
            state["postprocessor"] = OnlineQposPostprocessor(
                ...
                root_origin_offset=gmr_args.root_origin_offset,
                torch_device=gmr_args.torch_device,
                ground_clearance=_ground_clearance_for_robot(robot_key),
            )
    ...
    while True:
        try:
            ...
            # If all upstream producer threads are done and the queue is empty,
            # the stream is finished. Exit even if the end-of-stream None sentinel
            # was dropped (it can be lost when wham_Q is full); otherwise the main
            # loop spins forever and needs a manual Ctrl+C.
        ...
        if gvhmr_params is not None:
            ...
            if gmr_args is not None:
                ...
                # Only render GMR previews when TCP streaming is actually enabled
                # (--tcp).  Previously this was gated on tcp_stream_gmr alone, which
                # defaults to True, so the MuJoCo viewer was created and stepped even
                # in CSV-only runs -- needlessly pinning GPU memory that then OOM'd the
                # 8 GB card alongside WHAM + global SLAM.
                render_gmr_preview_this_frame = (
                    gmr_args.tcp and tcp_stream_gmr and
    ...
    if gmr_args is not None:
        # Drain GMR workers before stopping them: send the end-of-stream sentinel
        # first and let each worker finish every queued frame. Setting stop_event
        # first makes the worker loop exit immediately and silently drop the last
        # few queued frames, which freezes the tail of the recorded video / CSV.
        for state in gmr_states.values():
            q = state.get("worker_queue")
            if q is None:
                continue
    ...
    if slam_thread is not None:
        slam_thread.join(timeout=join_timeout)
        if slam_thread.is_alive():
            logger.warning("slam thread did not exit in time; continue shutdown.")
        if slam_dropped > 0:
            logger.info(f"SLAM dropped {slam_dropped} frame(s) (queue backpressure); "
                        "cam_angvel fell back to zeros for those windows.")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--global_slam", action="store_true",
                        help="Run DPVO global SLAM online and feed camera angular velocity into WHAM (default: off)")
